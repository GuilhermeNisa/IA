...
public class Graph {
	...
	public Node searchSolution3(String i, String f, String[] pr, Algorithms algID){

		int size = pr.length;

		Graph g = new Graph();

		Vertex start = this.vertices.get(i);
		Vertex finish = this.vertices.get(f);

		g.addVertex(i, start.getLatitude(), start.getLongitude());
		g.addVertex(f, finish.getLatitude(), finish.getLongitude());

		for(int j=0; j< size-1; j++){

			VertexSet p = this.getVertexSet(pr[j]);

			for (Vertex temp : p.getVertices()) {
	...
	public Node searchSolution2(String i, String f,String pr, Algorithms algID) {
		Graph g = new Graph();

		VertexSet p = this.getVertexSet(pr);
		...
		for (Vertex temp : p.getVertices()) {
			...
			Node sol2 = searchSolution(temp.getLabel(), f, algID);
			g.addEdge(temp.getLabel(), f, sol2.getPathCost());
		}
